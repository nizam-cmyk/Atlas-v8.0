import { classifyDocument, normaliseText as classifyNormaliseText } from '../lib/classify.js';
import { extractDocumentText } from '../lib/extract.js';
import { detectFormType } from '../lib/detectFormType.js';
import { buildSessionContext } from '../lib/buildSessionContext.js';
import { mergeContext } from '../lib/mergeContext.js';
...
import { loadHandbookPack } from '../lib/loadHandbookPack.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const uploadedDocument = previousContext?.uploadedDocument || null;

    const combined = [
      text,
      documentText,
      uploadedDocument?.title || '',
      uploadedDocument?.parsed?.programme || '',
    ...
    const combinedForIdentity = [
      ...
      uploadedDocument?.student?.studentId || ''
    ].filter(Boolean).join(' ');
    ...
    const studentThirdDigit =
      ...
      uploadedDocument?.student?.studentThirdDigit ||
      previousContext?.current_student_third_digit ||
      null;

    const intakeMatch =
      resolveIntakeFromStudentId(studentId, prefixMap) ||
      (uploadedDocument?.student?.intake
        ? {
            intake: uploadedDocument.student.intake,
            label: uploadedDocument.student.handbookLabel,
            version: uploadedDocument.student.handbookVersion
          }
        : null) ||
    ...
    const baseClassification = classifyDocument({
      message,
      filename: fileMeta?.filename || uploadedDocument?.filename || '',
      documentText: documentText || uploadedDocument?.title || ''
    });

    const formDetection = detectFormType({
      filename: fileMeta?.filename || uploadedDocument?.filename || '',
      documentText: documentText || uploadedDocument?.title || '',
      extractedTitle: extracted?.title || uploadedDocument?.title || '',
      topLines: extracted?.topLines || uploadedDocument?.extracted?.topLines || []
    });

    const classification = {
      ...baseClassification,
      formType: formDetection?.formType || uploadedDocument?.parsed?.formType || null,
      formConfidence: formDetection?.confidence ?? uploadedDocument?.classification?.formConfidence ?? null,
      formSignals: formDetection?.signals || uploadedDocument?.classification?.formSignals || []
    };
    ...
    if (!classification.documentType && uploadedDocument?.documentType) {
      classification.documentType = uploadedDocument.documentType;
    }

    const cgpa =
      extractCgpa(combined) ??
      uploadedDocument?.parsed?.cgpa ??
      previousContext?.last_cgpa ??
      null;

    const credits =
      extractCredits(combined) ??
      uploadedDocument?.parsed?.creditsCompleted ??
      previousContext?.last_credits ??
      null;

    const uploadedProgrammeHint = uploadedDocument?.parsed?.programme || null;

    const programme =
      detectProgramme([combined, uploadedProgrammeHint].filter(Boolean).join(' '), programmesData) ||
    ...
    switch (mode) {
      ...
      case 'unknown_upload':
        result = getUnknownUploadResponse(fileMeta || { filename: uploadedDocument?.filename || '' }, extracted, classification, handbookContext, uploadedDocument);
        break;
...
function detectMode({ text, combined, fileMeta, classification, previousContext }) {
  ...
  const formKeywords = [
    ...
    'withdrawal form', 'deferment', 'postponement', 'rof-', 'fee review'
  ];

  if (fileMeta?.filename || previousContext?.uploadedDocument?.documentType) {
    const activeDocumentType =
      classification?.documentType ||
      previousContext?.uploadedDocument?.documentType ||
      'unknown_upload';

    if (activeDocumentType === 'form') return 'form';
    if (activeDocumentType === 'transcript') {
      if (containsAny(text, graduationKeywords)) return 'graduation';
      if (containsAny(text, standingKeywords)) return 'standing';
      return 'transcript';
    }
    if (activeDocumentType === 'graduation_document') return 'graduation';
...
function getTranscriptResponse({ messageText, documentText, standingRules, graduationData, programmesData, handbookSections, programme, cgpa, credits, handbookContext, uploadedDocument }) {
  const combined = `${messageText} ${documentText}`.trim();
  const resolvedCgpa =
    Number.isFinite(cgpa) ? cgpa :
    uploadedDocument?.parsed?.cgpa != null ? uploadedDocument.parsed.cgpa :
    extractCgpa(combined);

  const resolvedCredits =
    Number.isFinite(credits) ? credits :
    uploadedDocument?.parsed?.creditsCompleted != null ? uploadedDocument.parsed.creditsCompleted :
    extractCredits(combined);

  const resolvedProgramme =
    programme ||
    detectProgramme([combined, uploadedDocument?.parsed?.programme || ''].filter(Boolean).join(' '), programmesData);
...
- Programme: ${resolvedProgramme ? resolvedProgramme.name : uploadedDocument?.parsed?.programme || 'Not identified'}
- CGPA: ${resolvedCgpa != null ? resolvedCgpa.toFixed(2) : 'Not identified'}
- Credits: ${resolvedCredits != null ? resolvedCredits : 'Not identified'}
- Student ID: ${uploadedDocument?.student?.studentId || handbookContext?.studentId || 'Not identified'}
...
function getUnknownUploadResponse(fileMeta, extracted, classification, handbookContext, uploadedDocument = null) {
...
Processing Note:
${uploadedDocument?.title ? `ATLAS detected the title "${uploadedDocument.title}", but that was still not enough for reliable routing.` : extractionNote}

Classification:
- documentType: ${classification?.documentType || uploadedDocument?.documentType || 'unknown_upload'}
- confidence: ${classification?.confidence != null ? classification.confidence : 'N/A'}
- formType: ${classification?.formType || uploadedDocument?.parsed?.formType || 'unknown_form'}

Important Note:
ATLAS can identify forms and transcripts more reliably when the uploaded PDF contains selectable text, not only scanned images.
...
- “I uploaded my transcript. Can I graduate?”
